...
def afficher():
    for x in range(grid_size[0]) :
        for y in range(grid_size[1]) :
            case = LabelFrame(canvas, bg = TILES_BG_COLOR[game_grid[x][y]], height = 200, width = 200)
            case.grid_propagate(0)
            case.grid(column = y, row = x)
            text = Label(case, text = str(game_grid[x][y]), bg = TILES_BG_COLOR[game_grid[x][y]], fg = TILES_FG_COLOR[game_grid[x][y]] , font = TILES_FONT)
            text.place(x=100, y=100, anchor="center")
